...
- Allow other classified read capabilities unless denied. An unknown or unclassified capability pauses the task and asks the owner through both the exact owner control conversation and local TUI. A reply in either place permanently approves the capability for that policy and resumes the task.
