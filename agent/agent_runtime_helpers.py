...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    try:
        ...
        if (new_provider or "").strip().lower() == "moa":
            ...
            # Merge custom/AIDE identity headers after route defaults so an
            # in-place switch preserves both sets and user values win.
            _sm_def_headers = _resolve_switch_default_headers(agent, new_provider)
            if _sm_def_headers:
                _merged_headers = dict(
                    agent._client_kwargs.get("default_headers") or {}
                )
                _merged_headers.update(_sm_def_headers)
                agent._client_kwargs["default_headers"] = _merged_headers
            agent.client = agent._create_openai_client(
