...
# Absolute / home-relative local image path. Matches POSIX ``/path`` and
# ``~/path`` forms plus native Windows drive paths. It ignores matches inside
# URLs (the lookbehind) and is case-insensitive on the extension.
...
def extract_image_refs(text: str) -> Tuple[List[str], List[str]]:
    ...
    Returns ``(local_paths, urls)``:
      ...
      * ``local_paths`` — absolute (POSIX or Windows) or home-relative
        (``~/``) paths
