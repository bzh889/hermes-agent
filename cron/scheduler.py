...
_HOME_TARGET_ENV_VARS = {
    ...
    "teams_mtk": "MTK_TEAMS_CONVERSATION_ID",
}
...
def _run_job_script(
    ...
    * anything else — run with the current Python interpreter
    ...
    execution_cwd = Path(workdir or path.parent).expanduser().resolve()
...
def run_job(
    ...
    if script_path:
        ...
        prerun_script = _run_job_script_with_claim_heartbeat(
            job, script_path, workdir=script_cwd,
        )
    ...
    # Use ContextVars for per-job cron/session/delivery state so jobs running
    # inside the gateway cannot leak cron approval mode into live conversations.
