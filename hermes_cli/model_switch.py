...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    ...
    headers = normalize_extra_headers(entry.get("default_headers"))
    headers.update(normalize_extra_headers(entry.get("extra_headers")))
    return headers
            ...
            if should_probe:
                try:
                    ...
                    live_models = fetch_api_models(
                        api_key,
                        api_url,
                        headers=_extra_headers_from_config(grp) or None,
                    )
                    if live_models:
