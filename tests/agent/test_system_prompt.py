...
def test_coding_prompt_preserves_legacy_workspace_order(monkeypatch):
    ...
    mocked_hermes_home = Path("/hermes")
    monkeypatch.setattr(system_prompt, "get_hermes_home", lambda: mocked_hermes_home)

    expected_profile = (
        "Active Hermes profile: default. Other profiles (if any) live "
        f"under {mocked_hermes_home}/profiles/<name>/. Each profile has its own skills/, "
