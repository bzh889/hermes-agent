...
- The default (no flags) is session-only; a plain ``/model <name>`` does
  not write to config.yaml.  Use ``--global`` to persist.
"""
...
class TestResolvePersistBehaviorGatewaySource:
    """``source="gateway"`` makes /model session-only by default.

    A model switch via the messaging gateway must NOT rewrite
    ``model.provider`` in config.yaml, because that config is the
    runtime source of truth for *all* gateway sessions across every
    platform.  Only ``--global`` overrides this.
    """

    def test_gateway_default_is_session_only(self):
        with _config({"model": {"persist_switch_by_default": True}}):
            assert resolve_persist_behavior(False, False, source="gateway") is False

    def test_gateway_global_overrides_source(self):
        with _config({"model": {"persist_switch_by_default": False}}):
            assert resolve_persist_behavior(True, False, source="gateway") is True

    def test_gateway_session_still_session_only(self):
        with _config({"model": {"persist_switch_by_default": True}}):
            assert resolve_persist_behavior(False, True, source="gateway") is False

    def test_cli_source_unchanged(self):
        # source="cli" (default) still defers to config.yaml.
        with _config({"model": {"persist_switch_by_default": False}}):
            assert resolve_persist_behavior(False, False, source="cli") is False
        with _config({"model": {"persist_switch_by_default": True}}):
            assert resolve_persist_behavior(False, False, source="cli") is True
