...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def capture(
        ...
        """Capture a filtered on-screen window or an exact known target.
            ...
            if not filtered:
                return self._failed_capture(
                    ...
                    (
                        ...
                        f"call list_apps/list_windows to see available app names, "
                        f"bundle IDs, and window titles "
        ...
        # Preserve a prior capture/focus target when it still matches the
        # caller's filters.
        active_target = next((
            w for w in windows
        ...
        target = active_target or _select_capture_target(
            windows,
            app_requested=bool(app or window_title),
    ...
    def list_apps(self) -> List[Dict[str, Any]]:
        ...
        # Preserve the legacy fallback's window-title enrichment. Canonical
        # structured app payloads above already carry the driver's full shape.
        try:
            windows = sorted(self._load_windows(), key=lambda w: w["z_index"])
        except Exception as exc:
            logger.debug(
                "computer_use: could not enrich legacy apps with windows: %s",
                exc,
            )
            return apps
    ...
    def focus_app(self, app: str, raise_window: bool = False,
        ...
        if window_title:
            ...
            exact_titles = [
                w for w in matched
                if str(w.get("title", "")).casefold() == title_lower
            ]
            matched = exact_titles or [
                w for w in matched
                if title_lower in str(w.get("title", "")).casefold()
            ]
